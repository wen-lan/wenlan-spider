	...
	def get_data(self):
		...
		while count < 20:
			...
			if cou>=10:
				...
				self.stop = -1
				break
			temp = [news_com_num,news_time]
			#test 如果是存在的 那么他就不等于()
			test = self.db.sql_execute('select n_num from newsdata where n_time=%s',news_time)
			if test!=():
				# 如果数据存在，那么就可以修改
				try:
					self.db.sql_execute('update newsdata set n_num=%s where n_time=%s',temp)
					# 这里修改必须要提交了
					self.db.conn.commit()
					print('修改了',news_name,'的评论数目','从',test[0][0],'到',news_com_num,'条')
					count += 1
					self.new_num += 1
				except:
					print(news_name,'数据修改失败，已经回滚，开始下一条数据')
					count += 1
					self.new_num += 1
